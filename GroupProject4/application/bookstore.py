...
def sell_book(title, quantity=1, sell_window = None):
    ...
    retruns a dict
    """
    with open('GroupProject4/files/books.csv', 'r') as booksFile:
    ...
    """
    with open('GroupProject4/files/books.csv', 'w', newline='') as booksFile:
    ...
    with open('GroupProject4/files/storefund.txt', 'r') as storeFile:
        store_fund = float(storeFile.read())
    with open('GroupProject4/files/storefund.txt', 'w') as storeFile:
    ...
    tk.Label(sell_window, text=f"Sell Book: {title}").pack(pady=20)
